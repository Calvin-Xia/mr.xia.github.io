...
本阶段已从方案评估进入实施并完成。目标是在不破坏静态站点、无框架膨胀、兼顾移动端的前提下，明确并落地值得做的文章阅读体验增强。

## Status

- 状态：已完成
- 完成日期：2026-05-03
- 验证：`npm test`、`npm run test:coverage`、`npm run build`、桌面与移动端浏览器检查均已通过
- 实现边界：文章增强逻辑集中在 `src/lib/article-enhancements/`，运行时入口为 `src/scripts/article-runtime.js`
...
- 图片灯箱纳入上一张/下一张切换，并保留 1x–4x 缩放、重置和关闭控制
- 目录移动端采用文章顶部折叠区，避免悬浮按钮遮挡正文
- 文章切换仅对 `/articles/` 与 `/updates/` 低风险站内路径启用，其他链接保留普通导航
