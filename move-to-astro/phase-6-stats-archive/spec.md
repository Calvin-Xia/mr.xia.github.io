...
- 状态：已完成；本地自动化、Astro 构建通过，Cloudflare 线上 `UMAMI_API_KEY` secret 已注入
...
- 在线阅读量：通过 Workers Static Assets 入口脚本代理 Umami Cloud API，前端渐进增强展示
...
- 生产环境：运行 `npx wrangler secret put UMAMI_API_KEY` 注入，真实 key 不写入仓库。
- 本地 Worker 调试：复制 `.dev.vars.example` 为 `.dev.vars` 后填入真实值；`.dev.vars*` 已加入 `.gitignore`。
- Astro `npm run dev` 不会执行 Worker API，浏览量徽章会在 `/api/views/*` 不可用时自动隐藏。
...
src/
├── lib/
│   ├── archive.js                 # 归档分组数据 helper
│   ├── umami-view-counter.js      # Umami API 代理核心逻辑
...
wrangler.jsonc                     # 修改：新增 "main": "src/worker.ts"、ASSETS binding 和 required secret
.dev.vars.example                  # 新增：本地 Worker secret 占位模板
tests/
